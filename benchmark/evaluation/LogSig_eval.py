...
if __name__ == "__main__":
    ...
    if args.freeze:
        # build suffix and summary file for the frozen evaluation
        ...
        for dataset in datasets:
            ...
            # set up parser for the frozen training and matching flow
            parser = LogParser(
                indir=indir,
                outdir=output_dir,
                groupNum=setting['groupNum'],
            ...
            # load the full log dataframe once
            ...
            # log the split counts
            print(f"{total_lines}, {train_size}, {test_size}")

            parse_start = time.time()

            # collect token sequences only from the training logs
            parser.wordLL = []
            for _, line in df_log.iloc[:train_size].iterrows():
                ...
                word_seq = content_line.strip().split()
                if len(word_seq) == 0:
                    word_seq = [' ']
                parser.wordLL.append(tuple(word_seq))

            # execute the original LogSig steps on the frozen training portion
            parser.termpairGene()
            parser.LogMessParti()
            parser.signatConstr()

            num_templates_before = len(parser.signature)
            print(num_templates_before)

            predictions = []

            # match each test log against the frozen signatures without updates
            for _, line in df_log.iloc[train_size:].iterrows():
            ...
            num_templates_after = len(parser.signature)
            ...
            # prepare ground truth and predictions using only the held-out test portion
        ...
        post_average(freeze_metric_file, f"LogSig_{data_type}_complex={args.complex}_frequent={args.frequent}{freeze_suffix}", args.complex, args.frequent)
